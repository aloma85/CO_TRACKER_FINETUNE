#!/usr/bin/env python3
"""
Custom training script for fine-tuning CoTracker on personal dataset.
This script uses pseudo-labeling approach with pre-trained teacher models.
"""
...
def forward_batch(batch, model, args, teacher_models):
    ...
    print(f"Teacher model output - trajs_g shape: {trajs_g.shape}, vis_g shape: {vis_g.shape}")
    ...
    if args.model_name == "cotracker":
        ...
        # Use the full sequence length instead of sliding windows
        # The model predicts for the entire sequence, so ground truth should match
        vis_gts.append(vis_g.float())
        invis_gts.append(1 - vis_g.float())
        traj_gts.append(trajs_g[:, :, :, :2])  # Full sequence, all trajectories, x,y coordinates
        valids_gts.append(valids * valid_mask)  # Full sequence

        # coord_predictions: [ [tensor, tensor, tensor, tensor] ]
        # We want: [ [tensor, tensor, tensor, tensor] ] (one segment, multiple iterations)
        coord_preds_formatted = coord_predictions

        print(f"coord_predictions: {len(coord_preds_formatted)}, traj_gts: {len(traj_gts)}, valids_gts: {len(valids_gts)}, vis_gts: {len(vis_gts)}")
        print(f"flow_pred: {coord_preds_formatted[0][0].shape}, flow_gt: {traj_gts[0].shape}")
        
        # Only compute loss if we have valid data
        if len(coord_preds_formatted) > 0 and len(traj_gts) > 0:
            seq_loss = sequence_loss(
                coord_preds_formatted,
                traj_gts,
                valids_gts,
                vis=vis_gts,
                #invis=invis_gts,
                loss_only_for_visible=args.loss_only_for_visible_pts,
            )
        else:
            # Create a dummy loss if no valid data
            seq_loss = torch.tensor(0.0, device=video.device, requires_grad=True)
            print("Warning: No valid data for loss computation, using dummy loss")
...
class Lite(LightningLite):
    def run(self, args):
        ...
        if self.global_rank == 0:
            ...
            # PATCH: Do NOT append extra datasets for evaluation
            # if not args.debug:
        ...
        # Use your custom dataset instead of the real_dataset
        train_dataset = CustomVideoDataset(
            video_dir=args.video_dir,  # New argument for video directory
        ...
        train_loader = DataLoader(
            ...
            shuffle=True,
            num_workers=min(args.num_workers, 4),  # Limit workers to prevent memory issues
        ...
        while should_keep_training:
            ...
            for i_batch, batch in enumerate(tqdm(train_loader)):
                try:
                    ...
                    print(f"Batch visibility shape: {batch.visibility.shape}")
                    ...
                    if self.global_rank == 0:
                        ...
                        if total_steps % save_freq == save_freq - 1:
                            ...
                            # Fix the extra dimension in predictions
                            pred_tracks = output["flow"]["predictions"]
                            if pred_tracks.dim() == 5:  # [B, 1, T, N, D]
                                pred_tracks = pred_tracks.squeeze(1)  # Remove extra dimension
                            
                            pred_visibility = output["visibility"]["predictions"]
                            if pred_visibility.dim() == 4:  # [B, 1, T, N]
                                pred_visibility = pred_visibility.squeeze(1)  # Remove extra dimension

                            visualizer.visualize(
                                video=batch.video.clone(),
                                tracks=pred_tracks,
                                visibility=pred_visibility > 0.6,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_root", type=str, help="path to all the datasets (train and eval)"
    )
    # New argument for video directory
    parser.add_argument(
        "--video_dir", type=str, required=True, help="path to your video directory"
    )
